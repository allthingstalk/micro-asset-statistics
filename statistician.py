...
class Statistician:
    ...
    def __init__(self, name, functions, resetEvery, asset):
        ...
        for function in functions:
            ...
            if name == "dist%":  # for std%, we also need std
                if "dist" not in self._functions:
                    ...
                    self._functions[name] = function                    # the parameters for std are the same as for dist%
            if name == "distsumtime%":                                  # for distsumtime%, we also need distsumtime
    ...
    def createAssets(self, context):
        ...
        if "dist%" in self._functions:
            Virtual.create(context, self._asset.device, self.getAssetName('dist%'), self.getAssetLabel('dist %'), "generated by the statistician. Distribution expressed in percentages", {"type": "array", "items": {"type": "number"}})
        ...
        if "distsumtime%" in self._functions:
            Virtual.create(context, self._asset.device, self.getAssetName('distsumtime%'), self.getAssetLabel('distsumtime %'), "generated by the statistician. Distribution expressed in percentages", {"type": "array", "items": {"type": "number"}})
    ...
    def try_calculate_dist_percent(self, context):
        res = Actuator(device=self._asset.device, name=self.getAssetName('dist%'), connection=self._asset.connection)
        dist = context['dist']
        total = sum(dist)
        res.value = [(i * 100) / total for i in dist]
        context['dist%'] = res.value

    def try_calculate_dist_sum_time_percent(self, context):
        res = Actuator(device=self._asset.device, name=self.getAssetName('distsumtime%'), connection=self._asset.connection)
        dist = context['distsumtime']
        total = sum(dist)
        res.value = [(i * 100) / total for i in dist]
        context['distsumtime%'] = res.value
    ...
    def resetValues(self):
        ...
        if "distsumtime" in self._functions:
            ...
            distSum.value = []
        if "distsumtime%" in self._functions:
            dist = Actuator(device=self._asset.device, name=self.getAssetName('distsumtime%'))
            dist.value = []
        if 'delta' in self._functions:
            deltaCur = Actuator(device=self._asset.device, name=self.getAssetName('deltaCurrentPeriod'))
            deltaPrev = Actuator(device=self._asset.device, name=self.getAssetName('deltaPrevTotal'))
